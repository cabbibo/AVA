...
Shader "UnityFinals/ImmatPedestal"
{
        ...
        Pass
        {
           ...
           struct v2f {
            ...
            v2f vert (float4 vertex : POSITION, float3 normal : NORMAL, float4 tangent : TANGENT, float2 uv : TEXCOORD0)
            {
                ...
                float n = noise( vertex * 10 );//sin(vertex.x * 100 + _Time * 10 ) + sin( vertex.y * 100 +  _Time * 10 );

                float3 fPos = vertex + normal * n * .1;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // output it!
                fixed4 c = 0;
                c.rgb = skyColor *  (worldNormal * .5 + .5);//skyColor;
                return c;
            }
            ENDCG
        }
