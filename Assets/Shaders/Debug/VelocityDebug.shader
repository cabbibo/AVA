...
//        Tags { "RenderType"="Transparent" "Queue" = "Transparent" }
    ...
    struct Vert{
        ...
        if( base < _Count ){
          ...
          if( alternate == 4 ){ extra =  l + u; uv = float2(1,1); }
        	...
        	Vert v = _VertBuffer[base % _Count];
       		o.worldPos = (v.pos) + extra * _Size * length(v.vel) * v.debug.x;///* .001/(.1+length(v.debug));//*(1/(.1+max(length(v.debug),0)));//mul( worldMat , float4( v.pos , 1.) ).xyz;
	        //o.debug = v.debug.x;
      ...
      float4 frag (varyings v) : COLOR {
          return float4(normalize(v.debug) * .5 + .5,1 );
      }
