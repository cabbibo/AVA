  ...
  SubShader {
    ...
    Pass {
      ...
      float getFogVal(float3 rd, float3 pos ){
        ...
        float v = (noise( pos * 3 +_Time.y * .3) + .5 * noise( pos * 8 -_Time.y * .2) + .25 * noise( pos * 14 + _Time.y ) );
     
        return 1-saturate( abs(v - 1) * 10);
      }
      ...
      VertexOut vert(VertexIn v) {
        ...
        o.rd = normalize(mPos - _WorldSpaceCameraPos);

        o.player = mPos - _Player;//v.position.xyz - mul( unity_WorldToObject , float4( _Player , 1. )).xyz;
        return o;
      ...
      fixed4 frag(VertexOut v) : COLOR {
        ...
        float lightM = dot(v.normal , _WorldSpaceLightPos0);
        float eyeM = dot(v.normal , rd);
        eyeM *= eyeM;
        ...
        for( int i = 0; i < _NumberSteps; i++ ){
          ...
          col += hsv( eyeM * .4 + stepVal *  _HueSize * (1 + _Hovered * .5 ) - _HueSize * .5* _Hovered + _BaseHue, .8 , val*val) ;
...
col *= saturate(lightM);
col += saturate(1-2*pow(eyeM,.3) + lightM);
		    fixed4 color;

		    if( abs(v.uv.x - .5) > .46 || abs(v.uv.y - .5) > .49 ){
		    	col = 0;
		    }
col *= fadeVal;
        //col = float3(1,0,0);
