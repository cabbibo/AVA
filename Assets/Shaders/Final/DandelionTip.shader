	...
	SubShader {
      ...
      float _BaseHue;
       ...
       color.xyz = tex2D(_ColorMap , float2( _BaseHue + v.uv.y * .2,0 )) * (v.uv.y * .5 + .8);;
        ...
        color = tcol;//sin(v.debug.x * 100);//tcol;// float4(0,1,0,1);//(tcol+1);
				if( tcol.a < .8 && v.debug.y < .5 ){ discard; }
       // if( v.debug.y < .3 ){ discard; }
        return float4( color.xyz * (shadow * .6 + .4), 1.);
			}
...
sampler2D _MainTex;
  struct v2f {
    ...
    o.debug = _TransferBuffer[id].debug;
        ...
        o.uv = _TransferBuffer[id].uv * .5 + offset;//.yx *float2(1./6.,1);;
        return o;
      }
      ...
      float4 frag(v2f v) : COLOR
      {
        ...
        if( col.a < .8 && v.debug.y < .5 ){ discard; }
