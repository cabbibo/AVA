    ...
    SubShader {
      ...
      float _BaseHue;
       ...
       color.xyz = tex2D(_ColorMap , float2( _BaseHue + v.uv.y * .2 ,0 )) * (v.uv.y * .5 + .8);;
        ...
        if( v.uv.y  > .8 ){ color.xyz = hsv(v.debug.x * .0004 + s* .1,1,1); }
        if( v.uv.y  < .1 ){ color.xyz = hsv(v.debug.x * .0004 + s* .1,1,1); }
        if( v.uv.x  < .1 ){ color.xyz = hsv(v.debug.x * .0004 + s* .1,1,1); }
        if( v.uv.x  > .8 ){ color.xyz = hsv(v.debug.x * .0004 + s* .1,1,1); }
