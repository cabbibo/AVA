...
float hVal = floor( hash(100*sin(idX*1212)+100*sin(idY*1021)) * 2 );
float hVal2 = floor( hash(100*sin(idX*12)+100*sin(idY*21)) * 2 );
            uv /= 2;
            uv += float2((1./2), (1./2)) * float2(hVal,hVal2);//offset;
        ...
        float a1 = asin(n.y / r);
         		...
         		col =  max(t1,tCol);// tex2D(_ColorMap, float2(length(tCol.x)  ,.2)) * .5 + .5;//hsv(t1.x * 1 + .3,1,1);

						col = tex2D(_ColorMap, float2(length(tCol) * .3 + _Time.y * .1 ,.2)) * .5 + .5;
         		//col = normalize(col);
