...
float eyeV = 0;
if( eyeM < 1 ){ eyeV = 1;}
                col =  tCol * hsv(sin(v.uv.x*10 + _Time) * .1 - eyeM * .1 -.2 ,1,1);//3 * tCol * abs( refl * .3 + .7) * _Color;//lerp(tCol , tex2D(_ColorMap , float2(pow( m,4) * 4 + _Swap * .3,0)) , .6+pow(m,10));// * (fCol * .3 + .7);
...
float eyeM = dot( normalize(v.eye) , n );
float eyeV = 0;
                col = 2* tCol * hsv(sin(v.uv.x*10 + _Time) * .1 + .1 + eyeM * .1,1,1);//3 * tCol * abs( refl * .3 + .7) * _Color;//lerp(tCol , tex2D(_ColorMap , float2(pow( m,4) * 4 + _Swap * .3,0)) , .6+pow(m,10));// * (fCol * .3 + .7);
