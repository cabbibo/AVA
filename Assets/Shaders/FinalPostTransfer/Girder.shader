      ...
      int _TransferCount;
        ...
        float2 fUV      = _TransferBuffer[id].uv;
            ...
            float4 frag(varyings v) : COLOR {
                ...
                float2 boltCalc =float2( abs(v.uv.x-.5) * 3  - .5, (abs(v.uv.y-.5) - .4) * 20  % 2);
                
                float2 boltDir= boltCalc - float2(.5,.5);
                float vVal = length( boltCalc - float2(.5,.5));
                boltDir = normalize( boltDir );

                float overall = 1-(vVal / .5);

                float4 nTex = tex2D(_NormalMap,float2(3,1) * .2*v.uv ) * 2 - 1;
    float3 n =  v.nor  + v.tan * boltDir.x * overall + v.bi * boltDir.y* overall;////normalize( normalize(float3( noise(v.worldPos*_NoiseSize) , noise(v.worldPos*_NoiseSize+10000) , noise(v.worldPos*_NoiseSize +100) -.5)) - 6*v.nor);
            n = normalize(n);
...
float eyeM = dot( normalize(v.eye) , n );
float eyeV = 0;
                col = length(tCol) * length(tCol)  * .5 ;//2* tCol * hsv(sin(v.uv.x*10 + _Time) * .1 + .1 + eyeM * .1,1,1);//3 * tCol * abs( refl * .3 + .7) * _Color;//lerp(tCol , tex2D(_ColorMap , float2(pow( m,4) * 4 + _Swap * .3,0)) , .6+pow(m,10));// * (fCol * .3 + .7);
                ...
                if( vVal > .5 ){ col *= .2;}// );vVal;// shadow * .5 + .5;
